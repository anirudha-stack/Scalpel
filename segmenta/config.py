...
class SegmentaConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SegmentaConfig":
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "SegmentaConfig":
        """Load configuration from YAML file."""
        file_path = Path(path)
        if not file_path.exists():
            raise SegmentaConfigError(f"Config file not found: {path}")

        try:
            with open(file_path, "r", encoding="utf-8") as f:
                data = yaml.safe_load(f)
        except yaml.YAMLError as e:
            raise SegmentaConfigError(f"Invalid YAML in config file: {e}")
