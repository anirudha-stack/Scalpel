...
class MicrotomeConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "MicrotomeConfig":
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "MicrotomeConfig":
        """Load configuration from YAML file."""
        file_path = Path(path)
        if not file_path.exists():
            raise MicrotomeConfigError(f"Config file not found: {path}")

        try:
            with open(file_path, "r", encoding="utf-8") as f:
                data = yaml.safe_load(f)
        except yaml.YAMLError as e:
            raise MicrotomeConfigError(f"Invalid YAML in config file: {e}")
